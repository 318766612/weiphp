...
//                 addWeixinLog($data,'syc_image_from_wechat'.get_pbid().'--4444');
    ...
    function file_lists()
    {
        $this->assign('normal_tips', '温馨提示：文件不能超过' . ini_get('upload_max_filesize') . '，支持apk,zip,rar,doc,docx,txt,xls,xlsx,pdf,ppt,pptx格式');
        $map ['is_use'] = 1;
    ...
    function file_add()
    {
        ...
        if (IS_POST) {
            ...
            $id = M('material_file')->insertGetId($data);
            if ($id) {
                $this->success('添加' . $model['title'] . '成功！', U('file_lists'));
            } else {
                $this->error('添加失败');
            }
    ...
    public function file_edit()
    {
        ...
        if (IS_POST) {
            ...
            if ($res !== false) {
                $this->success('保存' . $model['title'] . '成功！', U('file_lists'));
            } else {
                $this->error('保存失败');
            }
        } else {
