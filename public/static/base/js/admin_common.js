    ...
    */
    ...
    function initCopyBtn(id) {
        $('#' + id).click(function () {
            ...
            var $contentHtml = $('<div><div class="" style="padding:10px; height:530px;overflow-y:hidden;overflow-x:hidden;"><div class="mt_10">' +
    ...
    function selectPublicsPopup(target_url,btn_text) {
            var url = SELECT_PUBLICS_URL;
            url = urlMerge(url) + 'btn_text=' + encodeURIComponent(btn_text) + '&text=' + encodeURIComponent(target_url);

            console.log(url);

            var $contentHtml = $('<div><div class="" style="padding:10px; height:400px;overflow-y:hidden;overflow-x:hidden;"><div class="mt_10">' +
                '<iframe id="copyIframe" name="copyIframe" style="height:390px;width:100%; border:none" border="0" src="' + url + '"></iframe>' +
                '</div></div><div class="btn_bar">' +
                '<a href="javascript:;" class="border-btn cancel_btn">关闭</a></div></div>');

            $.Dialog.open("选择要操作的公众号", {width: 400, height: 510}, $contentHtml);
...
//							content = content.length>30?content.substring(0,30):content;
    ...
    var WeiPHP = {
        ...
        selectPublicsPopup:selectPublicsPopup,
